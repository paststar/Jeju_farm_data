...
def evaluate(transformer, loader1, loader2,epoch,best_acc, save_dir, _print, writer):
    #_print("evaluating...")
    start = time.time()
    ...
    with torch.no_grad():
        for step, tgt_data in enumerate(loader1):
    ...
    with torch.no_grad():
        for step, tgt_data in enumerate(loader2):
            tgt_imgs, tgt_labels = tgt_data
            tgt_imgs, tgt_labels = tgt_imgs.cuda(non_blocking=True), tgt_labels.cuda(non_blocking=True)
            tgt_preds = transformer(tgt_imgs)
            pred = tgt_preds.argmax(dim=1, keepdim=True)
